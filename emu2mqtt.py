...
## Licence
# This file is MIT (see LICENSE) EXCEPT for the portions noted under Attribution below,
# which remain under the Apache Licence 2.0. The repository is NOT MIT throughout:
# emu.py and api_classes.py come from Rainforest Automation's Emu-Serial-API, which
# publishes no licence at all. Read NOTICE before reusing anything here.

## Attribution
# Portions of this file come from [emu2influx](https://github.com/abaker/emu2influx)
# by Alex Baker, licensed under the Apache Licence 2.0 -- the EMU-2 message handling
# and its helpers (the Y2K constant, get_timestamp, get_reading, get_price) and the
...
def on_mqtt_connect(client, userdata, flags, result):
    if result == 0:
        ...
        # Announce availability here rather than in the publish loop: the value is
        # retained, so one publish per connection is enough for any subscriber
        # (including one that subscribes later), and the Last Will covers the
        # disconnected case. Publishing it every second only re-sent the same
        # retained byte string to the broker once a second for the life of the
        # process.
        client.publish(args.mqtt_topic + "/lwt", "online", int(args.mqtt_qos), True)
    else:
...
def main():
    ...
    # Only authenticate when a username was given; brokers configured for
    # anonymous access reject an empty username/password pair.
...
def parse_args():
    ...
    parser.add_argument("--mqtt_server", help="MQTT broker hostname or address", required=True)
    parser.add_argument("--mqtt_port", help="MQTT server port", required=False, default=1883)
    parser.add_argument("--mqtt_username", help="MQTT username (omit for an anonymous broker)", required=False, default='')
    parser.add_argument("--mqtt_password", help="MQTT password (omit for an anonymous broker)", required=False, default='')
