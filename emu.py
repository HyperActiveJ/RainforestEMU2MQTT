...
#
# NOTICE OF MODIFICATION: this copy has been MODIFIED from the original. The
# changes port the module from Python 2 to Python 3 -- print statements become
# print() calls, unicode() becomes str(), map() results are wrapped in list(),
# bytes read from the serial port are decoded to str before XML parsing, and the
# shebang names python3.
#
# One further change: the serial thread's debug copy of each outgoing command to
# 'static/emu-log.txt' has been REMOVED. It wrote into a directory this project
# does not ship, and its str.translate(None, ...) call raises TypeError under
# Python 3 inside a bare `except: pass`, so it never wrote anything. Nothing else
# in the serial thread changed.
#
